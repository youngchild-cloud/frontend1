...
const Goods = () => {
  ...
  const loadData = () => {
    //비동기 통신 사용
    axios
      .get('https://port-0-backend-express-server-mkvwe9x45fceba4b.sel3.cloudtype.app/goods')
      //성공시 데이터를 저장
      .then(res => {
        setData(res.data);
  ...
  const deleteData = (g_code) => { //매개변수로 g_code값 받는다.
    if (window.confirm('정말 삭제하시겠습니까?')) {
      axios //서버에 delete 요청을 전송
        .delete(`https://port-0-backend-express-server-mkvwe9x45fceba4b.sel3.cloudtype.app/goods/${g_code}`)
        //성공일때 아래 내용을 실행함.
        .then(() => {
